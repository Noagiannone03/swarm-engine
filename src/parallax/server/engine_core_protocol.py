...
FABI_ROUTE_ID_EXTRA_ARG = "fabi_route_id"
FABI_ROUTE_EPOCH_EXTRA_ARG = "fabi_route_epoch"
...
def _validate_supported_sampling_params(params: Mapping[str, Any]) -> None:
    ...
    allowed_extra_args = {
        ...
        FABI_ROUTE_ID_EXTRA_ARG,
        FABI_ROUTE_EPOCH_EXTRA_ARG,
    }
...
def routing_table_from_engine_core_sampling_params(
    ...
    if not isinstance(routing_table, (list, tuple)):
        raise UnsupportedEngineCoreField(
            f"EngineCoreSamplingParams.extra_args.{PARALLAX_ROUTING_TABLE_EXTRA_ARG} must be a list"
        )
