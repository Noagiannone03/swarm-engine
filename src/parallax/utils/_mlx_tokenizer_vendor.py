"""Vendored from ``mlx_lm.tokenizer_utils`` (Apple, MIT License).

Copied verbatim so Parallax's shared tokenizer path does not depend on the
``mlx_lm`` package, which transitively imports ``mlx.core`` (Apple-Silicon /
Linux only, absent on Windows where Parallax contributes via native vLLM).
This module is pure Python (``json`` + ``transformers``); it does not import mlx.
